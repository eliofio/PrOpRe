...
* **`Nstep`**: This is an optional argument (`-s/--step`) that determines the step size for decrementing the number of retained sites during the calculation of Resolution and Relevance points for each mapping. As shown in the flux diagram in **Figure 1**, starting from N<sub>atoms</sub> - 1, the number of retained sites decreases by the specified step value once the number of mappings (default: 50) is reached.

* **`Nstep`**: This is an optional argument (`-s/--step`) that describes the decrement in the number of sites to be retained, starting from N<sub>atoms</sub> - 1 during the calculation of Resolution and Relevance points for each mapping.  Starting from N<sub>atoms</sub> - 1, the number of retained sites decreases by the specified step value once the number of random mappings (default: _M = 50_) is reached. In other words, this parameter allows for controlling the granularity of the reduction in the number of retained sites during the calculation, considering the specific number of mappings conducted. Assuming that the exploration of Relevance and resolution points is maximized when decresing the number of sites by 1, on the other hand the value equals to 1 could not be the best choice since each time the RSD map must be computed and for big systems requires time. Thus, in order to decide the Nstep value Here are the two ways you can define this argument:
